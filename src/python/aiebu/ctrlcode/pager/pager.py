...
class Pager:
    ...
    def assignpagenumber(self, state, col, jobs, labels, content, text, data, page_index, pages, islastpage):
        ...
        pages.append(Page(col, page_index, text, data, islastpage))
    ...
    def pagify(self, state, col, content):
        """ pagify the content """
        ...
        for jobid in state.getjoblist():
            ...
            if type(jobid) == str and jobid[0:3] == 'eop':
                if len(page_jobs):
                    # conclude the page
                    self.assignpagenumber(state, col, page_jobs, page_labels, content, text, data, page_index, pages, 0)
            ...
            # the same job)
            tsize, jobs, labels = self.extractjobsandlabels(state, content, jobid, page_jobs)
        ...
        if len(page_jobs):
            self.assignpagenumber(state, col, page_jobs, page_labels, content, text, data, page_index, pages, 1)
        else:
            pages[page_index-1].islastpage = 1
        return pages
