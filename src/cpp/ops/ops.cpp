...
deserialize(ctrl_writer& writer, std::shared_ptr<disassembler_state> state, const char* data)
{
  assert(state->address() % align() == 0 && "uC DMA definition has to be 128-bit aligned!");
  std::string label = state->get_labels().at(state->address());
  ...
  while (ctrl_next_BD == 1) {
    ...
    arg.push_back(read_uint16_le(data + count));     // size field
    arg.push_back(read_uint16_le(data + count + 2)); // flags
    arg.push_back(read_uint32_le(data + count + 4)); // local_ptr_offset
    arg.push_back(read_uint32_le(data + count + 8)); // remote address high
    arg.push_back(read_uint32_le(data + count + 12));// remote address high
    ...
    uint32_t local_ptr_offset = arg[2] + state->address();
...
deserialize(ctrl_writer& writer, std::shared_ptr<disassembler_state> state, const char* data)
{
  ...
  for (uint32_t i = 0; i < count; ++i) {
    ...
    uint32_t val = read_uint32_le(data + pos);
    std::ostringstream oss;
