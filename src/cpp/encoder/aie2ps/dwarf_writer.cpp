...
// Producer string embedded in every CU — "aiebu <version>; ctrltext debug"
// aiebu_build_version is generated by cmake/version.cmake from the project VERSION field.
// Returned as a static local to satisfy cert-err58-cpp (avoids non-catchable throw at
// static storage duration initialization).
static const std::string& producer_str()
{
  static const std::string s = std::string("aiebu ") + aiebu_build_version + "; ctrltext debug";
  return s;
}
...
void dwarf_writer::append_u16(std::vector<uint8_t>& buf, uint16_t v)
{
  ...
  buf.resize(off + 2);
  boost::endian::store_little_u16(buf.data() + off, v);
}
...
void dwarf_writer::append_u32(std::vector<uint8_t>& buf, uint32_t v)
{
  const size_t off = buf.size();
  buf.resize(off + 4);
  boost::endian::store_little_u32(buf.data() + off, v);
}

void dwarf_writer::patch_u32(std::vector<uint8_t>& buf, size_t offset, uint32_t v)
{
  boost::endian::store_little_u32(buf.data() + offset, v);
}
...
{
  const auto offset = static_cast<uint32_t>(m_str_buf.size());
  const auto [it, inserted] = m_str_index.try_emplace(s, offset);
  if (!inserted)
    return it->second;
    ...
    const std::vector<std::shared_ptr<Function>>& col_funcs)
{
  ...
  auto intern_filename = [&](const std::string& fn) -> uint32_t {
    ...
    const auto idx = static_cast<uint32_t>(filenames.size());
  ...
  struct row_target { uint32_t addr; int32_t line_num; uint32_t file_idx; };
  auto emit_row = [&](row_target tgt) {
    const uint32_t addr     = tgt.addr;
    const int32_t  line_num = tgt.line_num;
    const uint32_t file_idx = tgt.file_idx;
    // Set file if changed
    ...
    if (line_enc >= 0 && line_enc < static_cast<int32_t>(LINE_RANGE) && addr_delta >= 0) {
      ...
      if (special >= static_cast<int32_t>(OPCODE_BASE) && special <= static_cast<int32_t>(DWARF_MAX_SPECIAL_OPCODE)) {
  ...
  // LLVM requires DW_LNCT_path in the format descriptor even when there is only
  // one (empty) compilation-directory entry.  Omitting the format descriptor
  // (format_count=0) causes llvm-dwarfdump to report "no path was found".
  // DWARF v5 spec §6.2.4: directory[0] is the compilation directory; we emit "".
  append_u8(prologue_body, 1);                         // directory_entry_format_count = 1
  append_uleb128(prologue_body, DW_LNCT_path);         // DW_LNCT_path
  append_uleb128(prologue_body, DW_FORM_string);       // DW_FORM_string (inline NUL)
  append_uleb128(prologue_body, 1);                    // directories_count = 1
  append_str(prologue_body, "");                       // directory[0] = "" (current dir)

  // DWARF v5 file name table: full paths stored inline as NUL-terminated strings.
  ...
  for (const auto& fn : filenames)
    append_str(prologue_body, fn);                      // inline NUL-terminated path
  ...
  // Patch header_length
  const auto header_length = static_cast<uint32_t>(prologue_body.size());
    ...
    const std::vector<annotation_type>& annotations)
{
  ...
  // Column module names
  std::vector<uint32_t> col_name_offs(cols.nums.size());
  for (size_t i = 0; i < cols.nums.size(); ++i)
    col_name_offs[i] = intern_str("column " + std::to_string(cols.nums[i]));
  ...
  for (size_t ci = 0; ci < cols.nums.size(); ++ci) {
    append_uleb128(dies, ABBREV_MODULE);
    append_u32(dies, col_name_offs[ci]);           // DW_AT_name       DW_FORM_strp
    append_u32(dies, cols.stmt_offsets[ci]);       // DW_AT_stmt_list  DW_FORM_sec_offset
    append_u32(dies, cols.byte_sizes[ci]);         // DW_AT_byte_size  DW_FORM_data4

    // DW_TAG_label children for annotated instructions
    for (const auto& func : cols.funcs[ci]) {
      for (const auto& line : func->get_textlines()) {
...
dwarf_writer::build(const std::string& cu_name, const Debug& debug)
{
  ...
  // by_col is a std::map so iteration is in ascending column-number order.
  std::vector<uint8_t> debug_line;
  col_data cols;
  const size_t ncols = by_col.size();
  cols.nums.reserve(ncols);
  cols.stmt_offsets.reserve(ncols);
  cols.byte_sizes.reserve(ncols);
  cols.funcs.reserve(ncols);

  for (auto& kv : by_col) {
    cols.nums.push_back(kv.first);
    cols.funcs.push_back(std::move(kv.second));
  }

  for (size_t i = 0; i < cols.nums.size(); ++i) {
    cols.stmt_offsets.push_back(static_cast<uint32_t>(debug_line.size()));

    auto stmt_list = build_line_table(cols.funcs[i]);
    ...
    uint32_t max_end = 0;
    for (const auto& func : cols.funcs[i]) {
      for (const auto& line : func->get_textlines()) {
    ...
    cols.byte_sizes.push_back(max_end);
  }

  // Build .debug_info
  auto debug_info = build_info(cu_name, cols, annotations);
