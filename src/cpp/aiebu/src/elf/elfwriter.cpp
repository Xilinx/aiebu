...
add_dynsym_section(ELFIO::string_section_accessor* stra, std::vector<symbol>& syms)
{
  ...
  for (auto & sym : syms) {
    ...
    sym.set_index(syma.add_symbol(*stra, sym.get_name().c_str(), 0, 0, ELFIO::STB_GLOBAL, ELFIO::STT_OBJECT, 0, sec->get_index()));
  }
...
add_reldyn_section(std::vector<symbol>& syms)
{
  ...
  for (auto & sym : syms) {
      rela.add_entry(sym.get_pos(), sym.get_index(), (unsigned char)ELFIO::R_X86_64_32,
                     (((ELFIO::Elf_Sxword)sym.get_schema() & SCHEMA_MASK) | ((sym.get_addend() & ADDEND_MASK) << ADDEND_SHIFT)));
  }
}
