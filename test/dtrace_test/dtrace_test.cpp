...
int run_dtrace_test(const std::string& dtrace_lib_path, const std::string& script_file, const std::string& map_file, const std::string& output_file) 
{
    ...
    if (!dtrace_handle) {
        std::cerr << "[ERROR]: failed to load dtrace library: " << dlerror() << std::endl;
        return 1;
    }

    // load and check functions from dtrace compiler
    auto get_dtrace_col_numbers = reinterpret_cast<get_dtrace_col_numbers_t>(dlsym(dtrace_handle, "get_dtrace_col_numbers_with_log"));
    auto get_dtrace_buffer_size = reinterpret_cast<get_dtrace_buffer_size_t>(dlsym(dtrace_handle, "get_dtrace_buffer_size"));
    auto populate_dtrace_buffer = reinterpret_cast<populate_dtrace_buffer_t>(dlsym(dtrace_handle, "populate_dtrace_buffer"));
    if (!get_dtrace_col_numbers || !get_dtrace_buffer_size || !populate_dtrace_buffer) {
        std::cerr << "[ERROR]: failed to load dtrace functions: " << dlerror() << std::endl;
    ...
    uint32_t status = get_dtrace_col_numbers(script_file.c_str(), map_file.c_str(), &buffers_length, dtrace_log_level);
    if (status != 0) {
    ...
    if (status == 0) 
    {
        ...
        if (dtrace_buffer_length > 0) 
        {
            ...
            control_buffer_file.write(reinterpret_cast<const char*>(dtrace_buffer.get()), dtrace_buffer_length * sizeof(uint32_t));
...
{
    if (argc != 5) {
        std::cerr << "Usage: " << argv[0] << "<dtrace_lib_path> <script_file> <map_file>" << std::endl;
        return 1;
    }
    ...
    std::vector<uint8_t> map_buffer((std::istreambuf_iterator<char>(map_file_stream)), std::istreambuf_iterator<char>());
